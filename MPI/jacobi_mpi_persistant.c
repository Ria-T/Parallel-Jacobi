...
//    printf("Input mits - maximum solver iterations:\n");
	...
	MPI_Request SRequests[2][4],RRequests[2][4];
    int u_case=0;

    t1 = MPI_Wtime();
	
	
	//Initializing persistant communication
	if(neighbors[UP] >= 0){
        MPI_Recv_init(&u_old[1], 1, table_row, neighbors[UP], 0, cart_comm, &RRequests[0][UP]);
        MPI_Send_init(&u_old[(n+2)+1], 1, table_row, neighbors[UP], 0, cart_comm, &SRequests[0][UP]);

        MPI_Recv_init(&u[1], 1, table_row, neighbors[UP], 0, cart_comm, &RRequests[1][UP]);
        MPI_Send_init(&u[(n+2)+1], 1, table_row, neighbors[UP], 0, cart_comm, &SRequests[1][UP]);
    }

    if(neighbors[DOWN] >= 0){
        MPI_Recv_init(&u_old[(m+1)*(n+2)+1], 1, table_row, neighbors[DOWN], 0, cart_comm, &RRequests[0][DOWN]);
        MPI_Send_init(&u_old[(m)*(n+2)+1], 1, table_row, neighbors[DOWN], 0, cart_comm, &SRequests[0][DOWN]);

        MPI_Recv_init(&u[(m+1)*(n+2)+1], 1, table_row, neighbors[DOWN], 0, cart_comm, &RRequests[1][DOWN]);
        MPI_Send_init(&u[(m)*(n+2)+1], 1, table_row, neighbors[DOWN], 0, cart_comm, &SRequests[1][DOWN]);
    }

    if(neighbors[LEFT] >= 0){
        MPI_Recv_init(&u_old[(n+2)], 1, table_column, neighbors[LEFT], 0, cart_comm,&RRequests[0][LEFT]);
        MPI_Send_init(&u_old[(n+2)+1], 1, table_column, neighbors[LEFT], 0, cart_comm,&SRequests[0][LEFT]);

        MPI_Recv_init(&u[(n+2)], 1, table_column, neighbors[LEFT], 0, cart_comm,&RRequests[1][LEFT]);
        MPI_Send_init(&u[(n+2)+1], 1, table_column, neighbors[LEFT], 0, cart_comm,&SRequests[1][LEFT]);
    }

    if(neighbors[RIGHT] >= 0){
        MPI_Recv_init(&u_old[2*n+3], 1, table_column, neighbors[RIGHT], 0, cart_comm,&RRequests[0][RIGHT]);
        MPI_Send_init(&u_old[(n+2)+n], 1, table_column, neighbors[RIGHT], 0, cart_comm,&SRequests[0][RIGHT]);

        MPI_Recv_init(&u[2*n+3], 1, table_column, neighbors[RIGHT], 0, cart_comm,&RRequests[1][RIGHT]);
        MPI_Send_init(&u[(n+2)+n], 1, table_column, neighbors[RIGHT], 0, cart_comm,&SRequests[1][RIGHT]);
    }
    ...
    while (iterationCount < maxIterationCount && error > maxAcceptableError)
    {
        ...
        if(neighbors[UP] >= 0){
            MPI_Start(&RRequests[u_case][UP]);
        }

        if(neighbors[DOWN] >= 0){
            MPI_Start(&RRequests[u_case][DOWN]);
        }

        if(neighbors[LEFT] >= 0){
            MPI_Start(&RRequests[u_case][LEFT]);
        }

        // And then sending table data to all neighbors (or simply those that exist)
        if(neighbors[RIGHT] >= 0){
            MPI_Start(&RRequests[u_case][RIGHT]);
        }

        if(neighbors[UP] >= 0){
            MPI_Start(&SRequests[u_case][UP]);
        }

        if(neighbors[DOWN] >= 0){
            MPI_Start(&SRequests[u_case][DOWN]);
        }

        if(neighbors[LEFT] >= 0){
            MPI_Start(&SRequests[u_case][LEFT]);
        }

        if(neighbors[RIGHT] >= 0){
            MPI_Start(&SRequests[u_case][RIGHT]);
        }
        ...
        if(neighbors[UP] >= 0) MPI_Wait(&RRequests[u_case][UP],MPI_STATUS_IGNORE);
        if(neighbors[DOWN] >= 0) MPI_Wait(&RRequests[u_case][DOWN],MPI_STATUS_IGNORE);
        if(neighbors[LEFT] >= 0) MPI_Wait(&RRequests[u_case][LEFT],MPI_STATUS_IGNORE);
        if(neighbors[RIGHT] >= 0) MPI_Wait(&RRequests[u_case][RIGHT],MPI_STATUS_IGNORE);
        ...
        if(neighbors[UP] >= 0) MPI_Wait(&SRequests[u_case][UP],MPI_STATUS_IGNORE);
        if(neighbors[DOWN] >= 0) MPI_Wait(&SRequests[u_case][DOWN],MPI_STATUS_IGNORE);
        if(neighbors[LEFT] >= 0) MPI_Wait(&SRequests[u_case][LEFT],MPI_STATUS_IGNORE);
        if(neighbors[RIGHT] >= 0) MPI_Wait(&SRequests[u_case][RIGHT],MPI_STATUS_IGNORE);
